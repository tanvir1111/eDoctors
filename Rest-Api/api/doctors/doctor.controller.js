...
const {getAllDoctors,loginDoctor,findDoctorById, findDoctorByPhone, register, resetPassword,updatePicture} = require('./doctor.service')
       ...
       console.log(req.body);
        loginDoctor(req.body, (err,results)=>{
            ...
            if(results.length==0){
                ...
                {
                    results[0].password = ""
                    const jsontoken= sign({result: results[0]},process.env.AUTH_TOKEN_SECRET,{
                        expiresIn:"25m"
                    })
    ...
    register: (req,res)=>{
        ...
        findDoctorById(req.body.doctor_id,(err,results)=>{
            ...
            register(req.body,(err,results)=>{
                if(err){
                    console.log(err);
                    return res.status(500).json({serverMsg: "Something went wrong", code: 0})
                }
                return res.status(200).json({serverMsg: "Registration Successfull", code: 1})
            })
        })
    },
